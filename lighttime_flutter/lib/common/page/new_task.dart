...
class _NewTaskState extends State<NewTask> {
  ...
  void initState() {
    ...
    _sponsor = new TextEditingController();
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: TextFormField(
                        controller: _sponsor,
                        validator: (value) => _getValidator(
                            value, S.of(context).SponsorEmailIsNull),
                        decoration: InputDecoration(
                            icon: Icon(Icons.person),
                            labelText: S.of(context).SponsorEmail),
                      ),
                    ),
                    Container(
...
// }
  ...
  sendTask() {
    ...
    if (newTaskState.validate() && _title.text.isNotEmpty) {
      ...
      task.sponsor = _sponsor.text;
      task.principal = Global.profile.user.email;
  ...
  _sendMessage(Task task) {
    ...
    // TaskChannel.getInstance().sendMessage(task.toSendMessage());
    var result = TaskWebSocketUtility().sendMessage(task.toSendMessage());
    if (result) {
      var snackBar = SnackBar(
      ...
      var snackBar = SnackBar(
        content: Text(S.of(context).TaskSendFail),
      );
      _scaffoldKey.currentState.showSnackBar(snackBar);
    }
  }
